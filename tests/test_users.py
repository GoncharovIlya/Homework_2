...
def test_user_creation_already_exist(client, user):
    ...
    user_data = {"username": "testuser1", "password": "1234"}
...
def test_user_delete(client, user, auth_headers):
    respose = client.delete(f'users/{user.id}', headers = auth_headers)
    assert respose.status_code == 200
    respose = client.delete(f'users/100', headers = auth_headers)
